    ...
    */
		...
		if (pixelOrder == "Row") {
			...
			for (let i = 0; i < this.imageService.r.length; i++) {
				...
				if (i % 25000 == 0) {
					this.loadingMessage =
						Math.floor((i * 100) / this.imageService.r.length).toString() + "%";
				...
				for (var colour of bitPlaneOrder) {
					...
					if (Object.keys(selectedBits).indexOf(colour) != -1) {
						...
						var extractedBits: string[] = pixelBinary.filter(
							(val, index) => selectedBits[colour].indexOf(7 - index) != -1
						);
		...
		//For columns
			...
			for (let c = 0; c < this.imageService.width; c++) {
				...
				if (c % 100 == 0) {
					this.loadingMessage =
						Math.floor((c * 100) / this.imageService.width).toString() + "%";
					await this.helpers.sleep(0);
				}
				//For each pixel row
				for (let r = 0; r < this.imageService.height; r++) {
					...
					for (var colour of bitPlaneOrder) {
						if (Object.keys(selectedBits).indexOf(colour) != -1) {
							...
							var pixelBinary: string[] = this.helpers
								.intToBin(this.imageService.rgba[index + colourIndex])
								.split("");
							//Extract only inputted bits
							var extractedBits: string[] = pixelBinary.filter(
								(val, index) => selectedBits[colour].indexOf(7 - index) != -1
