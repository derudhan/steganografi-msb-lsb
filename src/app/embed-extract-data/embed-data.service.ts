...
export class EmbedDataService {
	...
	async embed(
		...
		*/
		...
		if (padBits)
			toHide += "0".repeat(
				this.imageService.r.length * Object.values(selectedBits).flat().length -
					toHide.length
			);
		...
		if (pixelOrder == "Row") {
			...
			for (let i = 0; i < this.imageService.r.length; i++) {
				...
				if (i % 25000 == 0) {
					this.loadingMessage =
						Math.floor((toHidePos * 100) / toHide.length).toString() + "%";
				...
				for (var colour of bitPlaneOrder) {
					...
					if (Object.keys(selectedBits).indexOf(colour) != -1) {
						...
						var pixelBinary: string[] = this.helpers
							.intToBin(colourArrays[colour][i])
							.split("");
		...
		//For Columns:
				...
				for (let r = 0; r < this.imageService.height; r++) {
					...
					for (var colour of bitPlaneOrder) {
						if (Object.keys(selectedBits).indexOf(colour) != -1) {
							...
							var colourIndex: number =
								this.imageService.rgbaChars.indexOf(colour);

							//Get colour's binary value
							var pixelBinary: string[] = this.helpers
								.intToBin(copiedRgba[index + colourIndex])
								.split("");
							//Edit pixel with new value
							for (var hideIndex of selectedBits[colour]) {
								if (toHidePos < toHide.length) {
							...
							//Update pixel value
							copiedRgba[index + colourIndex] = parseInt(
								pixelBinary.join(""),
								2
							);
						}
			...
			return new ImageData(
				rgbaData,
				this.imageService.width,
				this.imageService.height
			);
		}
	}
}
