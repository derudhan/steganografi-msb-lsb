...
export class ImageService {
    ...
    */
			...
			pngObj.decode().then((data) => {
				...
				if (self.pngType == 3) {
					...
					for (let i = 0; i < self.pngPaletteColourArray.length; i += 3) {
						self.pngPaletteColourArrayGroups.push([
							self.pngPaletteColourArray[i],
							self.pngPaletteColourArray[i + 1],
							self.pngPaletteColourArray[i + 2],
						]);
					}
					self.r = self.pngPaletteIndexes.map(
						(index) => self.pngPaletteColourArrayGroups[index][0]
					);
					self.g = self.pngPaletteIndexes.map(
						(index) => self.pngPaletteColourArrayGroups[index][1]
					);
					self.b = self.pngPaletteIndexes.map(
						(index) => self.pngPaletteColourArrayGroups[index][2]
					);
					self.a = new Uint8ClampedArray(self.r.length).fill(255);
					...
					alert(`Tipe Png "${self.pngType}" masih belum di-support :(`);
				}
	...
	createImage(
		r: Uint8ClampedArray = this.r,
		g: Uint8ClampedArray = this.g,
		b: Uint8ClampedArray = this.b,
		a: Uint8ClampedArray = this.a
	): ImageData {
		/*
